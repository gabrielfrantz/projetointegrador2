<template>
  <v-container>
    ...
    <v-toolbar-items class="hidden-xs-only">
     <v-btn text  v-if="$store.state.isUserLoggedInAdm" @click="navigateTo({name: 'cursos'})">Cursos</v-btn>
    ...
    <div class="hiddden-sm-and-up">
     <v-menu offset-y v-if="$vuetify.breakpoint.xsOnly">
       ...
       <v-list class="responsiveMenu">
        <v-list-item v-if="$store.state.isUserLoggedIn"><v-btn text   @click="navigateTo({name: 'cursos'})">Cursos</v-btn></v-list-item>
